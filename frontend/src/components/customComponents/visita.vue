<template>
  ...
  <card v-if="!isLoadingImages" :style="[
    ...
    <div class="mt-3">
      <h5 class="card-title text-center">Visita {{ item["geo:Titolo_it"][0]["@value"] }}</h5>
      ...
      <div :class="{ 'text-justify ': !isMobile }">
        ...
        <div v-else-if="selectedTab === 'Immagini'" class="col-12 text-center">
          <swiper v-if="item.mediaImages !== undefined && item.mediaImages.length !== 0" ref="mySwiper" class="swiper" navigation
            :pagination="{ clickable: true }" style="background-color: hsl(17, 100%, 90%); border-radius: 10px">
            <swiper-slide v-for="(media, index) in item.mediaImages" :key="index"
              :style="[isLarge ? { height: 'fit-content', maxHeight: '280px' } : { height: 'fit-content', maxHeight: '199px' }]">
              <img v-img="{group: indexOfActivity}" :src="media['o:thumbnail_urls']['large']" :alt="item['o:title']" class="img" />
            </swiper-slide>

            

            <div v-if="item.mediaImages.length > 1" class="swiper-button-prev" slot="button-prev" @click="swiper.slidePrev()">
            </div>
            <div v-if="item.mediaImages.length > 1" class="swiper-button-next" slot="button-next" @click="swiper.slideNext()">
            </div>
          </swiper>

          <div v-if="item.mediaImages === undefined || item.mediaImages.length === 0" class="mt-3 text-black">
